...
function pageSetup() {
  ...
  xhr.send();
  document.getElementById("quadrant").addEventListener(
    "keyup",
    function () {
...
function searchByName(da) {
  ...
  for (var i = 0; i < parsedrecord.length; i++) {
    ...
    //check
    event = record.all_dates; //assign
    if (event.toLowerCase().startsWith(da.toLowerCase())) {
      ...
      if (record.more_info_url != null) {
        output +=
          "<a href=https://" + record.more_info_url + ">" + record.more_info_url + "</a>";
